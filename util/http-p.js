...
class HTTP{
  request({url,method="GET",data={}}){
    return new Promise((resolved,rejected)=>{
      this._request(url,resolved,method,data)
    })
  }
  ...
  */
  _request(url,resolved,method="GET",data={}){
    wx.request({
      ...
      success:(res)=>{
        ...
        if( statusCode.startsWith('2') ){
          //有时不需要执行回调函数，也就没有传递success，此处如不先判断是否有success，在不存在回调success回调函数时，执行success会报错
          resolved(res.data);
        }else{
          rejected();
          this._show_err(res)
